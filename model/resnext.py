...
class UNet_SeResnext(nn.Module):
    def __init__(self, num_classes=1, num_filters=32, encoder_type = 'resnext50', pretrained=True, is_deconv=False):
        super().__init__()
        ...
        encoders = {
            'resnext50': se_resnext50_32x4d,
            'resnext101':se_resnext101_32x4d,
        }

        if pretrained:
            self.encoder = encoders[encoder_type](num_classes=1000, pretrained='imagenet')
        else:
            self.encoder = encoders[encoder_type](num_classes=1000, pretrained=None)
    ...
    def forward(self, x):
        ...
        dec1 = self.dec1(torch.cat([dec2,  Interpolate(scale_factor=2)(conv1)], 1))
        dec0 = self.dec0(dec1)
